...
if G_WinAlreadyTriggered == true then return end
	...
	if attackers == TEAM_RED then
		...
		for i, ply in ipairs( player.GetAll() ) do
			...
			EmitSound( Sound( "ambient/alarms/warningbell1.wav" ), ply:GetPos(), 1, CHAN_AUTO, 1, 75, 0, 100 )
		end
	end
...
function WinningPhase(winners)
	...
	for k,v in pairs(player.GetAll()) do
		...
		EmitSound( Sound( "ambient/alarms/warningbell1.wav" ), Entity(1):GetPos(), 1, CHAN_AUTO, 1, 75, 0, 100 )
